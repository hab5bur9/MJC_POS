...
public class MainMenu_Frame extends JFrame implements Runnable{
	...
	MainMenu_Frame(){
		...
		lab_info = new JLabel[3]; // index 0 => 현재 시간, index 1 => 근무 시간, index 2 => 근무자 코드
		...
		String[] btnNames2 = {"관리자 전용", "되돌리기", "캘린더", "직원 관리", "만든이들", "매출 조회"};
	...
	//실시간 시간 출력을 위한 run 메소드
	public void run(){
		while(true){
			Calendar cal = Calendar.getInstance();
			int amPm = cal.get(Calendar.AM_PM);//오전/오후를 받아온다
			String ampm=amPm==Calendar.AM? "오전":"오후";//비교해서 pm이나 am을 ampm에 저장
			String now = ampm + " " + cal.get(Calendar.HOUR)+"시"+cal.get(Calendar.MINUTE)+"분"+cal.get(Calendar.SECOND)+"초";
			lab_info[0].setText(now);
			try{
				Thread.sleep(1000);
